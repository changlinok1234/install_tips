...
\chead{\color{black!50!green}Ubuntu14.04安装}
...
%%---------------------------------------------------------------------
% \titlepage
\title{\vspace{-2em}Ubuntu14.04安装\vspace{-0.7em}}
\author{常琳（整理）}
...
\item 之后会出现分区界面，单机左下方的“-”会删除分区，“+”会出现创建分区对话框，实验室一般分/boot（单系统不用）、/swap（内存4G以上不用考虑）、/、/home、/opt（网上查得的先后顺序，不一定百分百正确，目前使用无状况）。

/boot 一般分100-200M，/swap为内存的1.5-2倍。以800G硬盘空间为例，在创建分区对话框中可以根据表\ref{tab:1}划分大小：
...
挂载点&大小&用于\\
\hline
boot&250M&Ext4日志文件系统\\
\hline
/swap&8G&交换空间\\
\hline
/&100G&Ext4日志文件系统\\
\hline
/home&400G &Ext4日志文件系统\\
\hline
/opt&剩下的292G &Ext4日志文件系统\\
\hline
\end{tabular}  
\caption{分区大小}
\end{table}
